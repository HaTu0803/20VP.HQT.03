...
export default {
    ...
    async findAllHopDongChuaKy() {
        const sql = `exec EX_11_READ_DONDK_PHANTOM_FIX `
        try {
            const list = await db.raw(sql)
            console.log(list);
            return list
    ...
    async findAllNhanVien() {
        const sql = `exec EX_2_READ_INFO_UNCOMMITTED_READ_FIX`
        try {
            const list = await db.raw(sql)
            console.log(list);
            return list
    ...
    async NhanVienNhanDonDK(maddk,manv) {
        ...
        const storedProcedureName = manv === 'NV_02' ? 'EX_22_UPDATE_DONDKA_LOST_UPDATE_FIX' : 'EX_22_UPDATE_DONDKB_LOST_UPDATE_FIX';
