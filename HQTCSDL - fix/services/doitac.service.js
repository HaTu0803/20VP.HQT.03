...
export default {
    ...
    async getTranhChap11(nguoidd,email,sdt,tennh,diachinh,stk,tenquan,thanhpho,quan,diachi,loaiamthuc,sldonhangmn) {
        const sql = `exec EX_11_INSERT_DONDK_PHANTOM_FIX N'${nguoidd}', N'${tennh}',N'${diachinh}','${stk}','${email}',N'${tenquan}',N'${thanhpho}',N'${quan}',N'${diachi}',${sldonhangmn},N'${loaiamthuc}','${sdt}',0`
        console.log(sql)
        try {
            await db.raw(sql)
        } catch (error) {
    ...
    async HuyCapNhatGia(mama,matd,gia) {
        const sql = `exec EX_1_UPDATE_INFO_UNCOMMITTED_READ_FIX '${mama}','${matd}','${gia}'`
        try {
            await db.raw(sql)
            console.log(sql)
        } catch (error) {
    ...
    async HuyCapNhatTen(mama,matd,tenma) {
        const sql = `exec EX_5_UPDATE_MONAN_UNREPEATABLE_READ_FIX '${mama}','${matd}','${tenma}'`
        console.log(sql)
        try {
            await db.raw(sql)
        } catch (error) {
    ...
    async get_TranhChap6(mama,matd,gia) {
        const sql = `exec EX_6_UPDATE_MONAN_UNREPEATABLE_READ_FIX '${mama}','${matd}','${gia}'`
        console.log(sql)
        try {
            await db.raw(sql)
        } catch (error) {
    ...
    async tranhchap9(matd,mama,tenma,mieuta,gia) {
        const sql = `exec EX_9_READ_MONAN_PHANTOM_FIX '${matd}','${mama}',N'${tenma}',N'${mieuta}','${gia}'`
        try {
            await db.raw(sql)
            console.log(sql)
        } catch (error) {
